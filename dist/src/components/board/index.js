...
const Board = (props) => {
    const { boardHeight, boardWidth, className, columns, image, onPuzzleComplete, outlineStrokeColor, puzzlePieceOptions, rows, showBoardSlotOutlines, snapThreshold, scatterArea, onAnyPieceActiveChange, } = props;
    ...
    const [snappedPieceIds, setSnappedPieceIds] = useState(new Set());
    ...
    const boardPathOptions = useMemo(() => ({
        boardHeight,
        boardWidth,
        columns,
        edgeMap,
    ...
    const handlePieceSnap = (pieceIndex) => {
    ...
    const handleSnapWithKeyboard = (currentPieceIndex) => {
    ...
    const registerPieceRef = (boardSlotKey, ref) => {
        if (ref) {
    ...
    return (_jsxs("svg", { ref: boardRef, className: `${styles.board} ${className}`, "data-testid": "board", height: boardHeight, width: boardWidth, viewBox: `0 0 ${boardWidth} ${boardHeight}`, children: [_jsx(BoardOutlines, { boardPathOptions: boardPathOptions, boardSlots: boardSlots, showBoardSlotOutlines: showBoardSlotOutlines, snappedPieceIds: snappedPieceIds }), shuffledPieces.map(({ pieceRow, pieceCol, x, y }, pieceIndex) => (_jsx(PuzzlePiece, { boardHeight: boardHeight, boardWidth: boardWidth, boardSlotKey: `${pieceRow}-${pieceCol}`, image: image, pieceIndex: pieceIndex, initialX: x, initialY: y, onSnap: () => handlePieceSnap(pieceIndex), onSnapWithKeyboard: () => handleSnapWithKeyboard(pieceIndex), path: generateBoardPath({ col: pieceCol, row: pieceRow, options: boardPathOptions }), puzzlePieceOptions: puzzlePieceOptions, registerPieceRef: registerPieceRef, snapThreshold: snapThreshold, boardRef: boardRef, targetX: (pieceCol * pieceWidth) / 100, targetY: (pieceRow * pieceHeight) / 100, onDragStart: () => onAnyPieceActiveChange?.(true), onDragEnd: () => onAnyPieceActiveChange?.(false) }, `${pieceRow}-${pieceCol}`)))] }));
};
