 ...
 *   <g ref={ref} {...handlers} ... />
 */
export function useMovePieces({ initialX, initialY, boardRef, targetX, targetY, snapThreshold, onSnap, onDragStart, onDragEnd, }) {
    ...
    const checkSnap = useCallback((x, y) => {
    ...
    const handlePointerMove = useCallback((e) => {
        ...
        if (!dragInfo || !dragInfo.isDragging || e.pointerId !== dragInfo.pointerId)
            return;
    ...
    const handlePointerMoveElement = useCallback((event) => {
        event.preventDefault();
    }, []);
    const handlePointerUpElement = useCallback((event) => {
        event.preventDefault();
    }, []);
...
//# sourceMappingURL=use-move-pieces.js.map